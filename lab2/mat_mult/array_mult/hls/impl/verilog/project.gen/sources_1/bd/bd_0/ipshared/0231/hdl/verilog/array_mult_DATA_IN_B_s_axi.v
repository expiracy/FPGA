...
//------------------------Instantiation------------------
// int_in_b
array_mult_DATA_IN_B_s_axi_ram #(
    .MEM_STYLE ( "auto" ),
    .MEM_TYPE  ( "2P" ),
    .BYTES     ( 4 ),
    .DEPTH     ( 25 )
...
assign RVALID  = (rstate == RDDATA) & !int_in_b_read;
...
assign int_in_b_d1       = WDATA;
// int_in_b_read
always @(posedge ACLK) begin
    if (ARESET)
        int_in_b_read <= 1'b0;
    else if (ACLK_EN) begin
        if (ar_hs && raddr >= ADDR_IN_B_BASE && raddr <= ADDR_IN_B_HIGH)
            int_in_b_read <= 1'b1;
        else
            int_in_b_read <= 1'b0;
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (aw_hs && AWADDR[ADDR_BITS-1:0] >= ADDR_IN_B_BASE && AWADDR[ADDR_BITS-1:0] <= ADDR_IN_B_HIGH)
            int_in_b_write <= 1'b1;
        else if (w_hs)
            int_in_b_write <= 1'b0;
    end
...
module array_mult_DATA_IN_B_s_axi_ram
#(parameter
    MEM_STYLE = "auto",
    MEM_TYPE  = "S2P",
    BYTES  = 4,
