...
architecture behave of array_mult_DATA_IN_B_s_axi is
    ...
    signal int_in_b_d1         : UNSIGNED(31 downto 0);
    ...
    end component array_mult_DATA_IN_B_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_in_b
int_in_b : array_mult_DATA_IN_B_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 4,
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_in_b_read = '0') else '0';
    RVALID    <= RVALID_t;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    rdata_data <= (others => '0');
                elsif (int_in_b_read = '1') then
                    rdata_data <= int_in_b_q1;
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (ar_hs = '1' and raddr >= ADDR_IN_B_BASE and raddr <= ADDR_IN_B_HIGH) then
                    int_in_b_read <= '1';
                else
                    int_in_b_read <= '0';
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (aw_hs = '1' and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) >= ADDR_IN_B_BASE and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) <= ADDR_IN_B_HIGH) then
                    int_in_b_write <= '1';
                elsif (w_hs = '1') then
                    int_in_b_write <= '0';
                end if;
...
entity array_mult_DATA_IN_B_s_axi_ram is
    generic (
        MEM_STYLE : STRING :="auto";
        MEM_TYPE  : STRING :="S2P";
        BYTES   : INTEGER :=4;
...
end entity array_mult_DATA_IN_B_s_axi_ram;

architecture behave of array_mult_DATA_IN_B_s_axi_ram is
